...
| portable-docker-instance | done | a root Dockerfile, local Compose overlay, remote GitHub Compose profile, npm helpers, tests, and README/local-platform docs now let a checkout, vendored copy, or no-checkout Docker Compose command rebuild and start a local public runtime without changing the existing single-node dependency-stack semantics, while keeping the UI demo behind an explicit Docker profile |
| durable-public-runtime-assembly | done | production-like public runtime startup now resolves to durable state by default, refuses local control-plane state, runs registry migrations plus idempotent namespace/delivery-scope bootstrap, wires Prisma registry stores, tusd upload targets, and S3-compatible source promotion, and documents object-store source evidence as a small-deployment durability lane without claiming dedupe |
| split-history-artifact-policy | done | generic archive adapters, policy docs, tests, and the corpus benchmark harness now prove the current-version-whole plus historical-version-split strategy without hard-coding the behavior to Unity assets |
