...
CDNgine's runtime auth contract should be the `@cdngine/auth` bearer-token authenticator boundary.

The repository's default adapter uses **Better Auth**, but Better Auth is **not** the required platform contract. A deployment may use any identity or session system that can validate the caller and map the result into a CDNgine `AuthenticatedActor`.

The minimum supported auth posture is:

- bearer-token authenticated requests following standard `Authorization: Bearer ...` semantics
