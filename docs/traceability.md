...
| `packages/registry/src/*publication-store.ts` and `packages/registry/src/prisma-public-version-read-store.ts` | project canonical versions into processing and published control-plane state with deterministic upsert semantics, then issue source and delivery grants through durable idempotency evidence | `docs/domain-model.md`, `docs/persistence-model.md`, `docs/state-machines.md`, `docs/idempotency-and-dispatch.md` | `packages/registry/test/image-publication-store.test.mjs`, `packages/registry/test/presentation-publication-store.test.mjs`, `packages/registry/test/prisma-public-version-read-store.test.mjs` | aligned; publication records remain durable registry truth, and public-read retries replay grants instead of minting duplicate one-time delivery capabilities |
